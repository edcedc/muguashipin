...
public class IncomeFrg extends BaseFragment<IncomePresenter, FIncomeBinding> implements IncomeContract.View, View.OnClickListener {
    ...
    protected void initView(View view) {
        ...
        mB.tvPerformance.setText("0" +
                " 钻石");
        mB.tvBalance.setText("0" +
                " 钻石");
        mB.tvWithdrawable.setText("0" +
                " 元");
        mB.tvPayment.setText("5.0" +
                "%");
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.bt_submit:
                mPresenter.onSubmit(mB.etName.getText().toString(), mB.etMoble.getText().toString(), mB.etBalance.getText().toString());
                break;
        }
    }
}
